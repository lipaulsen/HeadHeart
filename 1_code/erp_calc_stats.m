...
if ismember('Plot Signif Clustering by Med', steps)
    ...
    for s = 1:length(chantags)
        ...
        field_on = fields{contains(fields, 'MedOn')};
        field_off = fields{contains(fields, 'MedOff')};
        subs_on = Cluster.Map_Med_On.Subject; 
        subs_off = Cluster.Map_Med_Off.Subject;
        raw_data_on = Cluster.(field_on);
        raw_data_off = Cluster.(field_off);

         % Get unique subjects
        usubs_on = unique(subs_on);
        usubs_off = unique(subs_off);

        % Preallocate subject × time matrices
        data_on = nan(numel(usubs_on), size(raw_data_on, 2));
        data_off  = nan(numel(usubs_off), size(raw_data_off, 2));

        % Average across channels within subject
        for si = 1:numel(usubs_on)
            data_on(si,:) = mean(raw_data_on(strcmp(subs_on, usubs_on{si}), :), 1); 
        end
        for so = 1:numel(usubs_off)
            data_off(so,:) = mean(raw_data_off(strcmp(subs_off, usubs_off{so}), :), 1);
        end

        %optional save
         save(fullfile(data_dir, 'erp', 'group', 'clustering', ['Cluster_Data_ON_OFF_Mean-by-Sub_4Perm_',chantag,'.mat']), 'data_on', 'data_off', '-v7.3')

         [common_subs, idx_on, idx_off] = intersect(usubs_on, usubs_off);

         data_on_common = data_on(idx_on, :);
         data_off_common = data_off(idx_off, :);

         % Mean + SEM
         data_on_mean = mean(data_on_common,1);
         data_on_sem = std(data_on_common) / sqrt(length(data_on_common));

         data_off_mean = mean(data_off_common,1);
         data_off_sem = std(data_off_common,1) / sqrt(length(data_off_common));
         ...
         sig_time = find(pvals < 0.05);
         sig_times = AVGECG.times(sig_time);
        ...
        % --- MedOn shaded SEM + line ---
        fill([t, fliplr(t)], ...
            [data_on_mean(31:end) + data_on_sem(31:end), ...
            fliplr(data_on_mean(31:end) - data_on_sem(31:end))], ...
            [0.2 0.6 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        h1 = plot(t, data_on_mean(31:end), 'Color', [0 0.3 0.9], 'LineWidth', 2);

        % --- MedOff shaded SEM + line ---
        fill([t, fliplr(t)], ...
            [data_off_mean(31:end) + data_off_sem(31:end), ...
            fliplr(data_off_mean(31:end) - data_off_sem(31:end))], ...
            [1 0.4 0.4], 'FaceAlpha', 0.3, 'EdgeColor', 'none');
        h2 = plot(t, data_off_mean(31:end), 'Color', [0.9 0.1 0.1], 'LineWidth', 2);

        % --- Variables for significance detection ---
        is_significant = pvals(31:end) < 0.05; % logical vector
        ybounds = [min([data_on_mean data_off_mean], [], 'all'), ...
            max([data_on_mean data_off_mean], [], 'all')];

        % Make sure vectors are row shape
        is_significant = reshape(is_significant, 1, length(is_significant));
        t = reshape(t, 1, length(t));

        if length(t) ~= length(is_significant)
            error('t and Significance vectors must have same length.')
        end

        % --- Find significant time start/stop ---
        sig_onset = find(diff(is_significant) == 1) + 1; % where significant period starts
        sig_offset = find(diff(is_significant) == -1);   % where significant period stops
        ...
        % --- Plot significance shading (drawn first so it’s behind SEM lines) ---
        for k = 1:length(sig_onset)
            fill([t(sig_onset(k)) t(sig_offset(k)) ...
                t(sig_offset(k)) t(sig_onset(k))], ...
                [ybounds(1) ybounds(1) ybounds(2) ybounds(2)], ...
                'k', 'FaceAlpha', 0.2, 'EdgeAlpha', 0.2);
        end
        ...
        gr2 = fullfile('E:', 'HeadHeart', '2_results', 'erp', 'clustering', [ 'Hierarchical-Clustering_SIGNIF_Comparison_MedOn-MedOff_HEP_',char(chantag),'.png']);
        exportgraphics(f2, gr2, "Resolution", 300)
...
if ismember('Plot Signif Clustering by Method', steps)
    meds = {'MedOn' 'MedOff'};
    for m = 1:length(meds)
        med = meds{m};
        ...
        filename = fullfile(files(1).folder, files(1).name);
        load(filename, 'Cluster');
        subs_eeg = Cluster.Map_Med_On.Subject; % currently always MedOn - Change for Med OFF !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        % Filter correct field
        fields = fieldnames(Cluster);
        field_eeg = fields{contains(fields, med)};
        raw_data_eeg = Cluster.(field_eeg);
        ...
        filename = fullfile(files(1).folder, files(1).name);
        load(filename, 'Cluster')
        subs_stn = Cluster.Map_Med_On.Subject; % currently always MedOn - Change for Med OFF !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        % Filter correct field
        fields = fieldnames(Cluster);
        field_stn = fields{contains(fields, med)};
        raw_data_stn = Cluster.(field_stn);

        % Get unique subjects
        usubs_eeg = unique(subs_eeg);
        usubs_stn = unique(subs_stn);
        if strcmp(med, 'MedOff')
            [usubs, idx_eeg, idx_stn] = intersect(usubs_eeg, usubs_stn);
            raw_data_stn = raw_data_stn(idx_stn, :);
            raw_data_eeg = raw_data_eeg(idx_eeg, :);
            subs_stn = subs_stn(idx_stn);
            subs_eeg = subs_eeg(idx_eeg);
        end

        % Preallocate subject × time matrices
        data_eeg = nan(numel(usubs_eeg), size(raw_data_eeg, 2));
        data_stn  = nan(numel(usubs_stn), size(raw_data_stn, 2));

        % Average across channels within subject
        for si = 1:numel(usubs_eeg)
            data_eeg(si,:) = mean(raw_data_eeg(strcmp(subs_eeg, usubs_eeg{si}), :), 1);
            data_stn(si,:) = mean(raw_data_stn(strcmp(subs_stn, usubs_stn{si}), :), 1);
        end

        % Mean + SEM EEG
        data_eeg_mean = mean(data_eeg,1, "omitnan");
        data_eeg_sem = std(data_eeg, "omitnan") / sqrt(length(data_eeg));
        % Mean + SEM STN
        data_stn_mean = mean(data_stn,1, "omitnan");
        data_stn_sem = std(data_stn) / sqrt(length(data_stn));

        %optional save
        % save(fullfile(data_dir, 'erp', 'group', 'clustering', ['Cluster_Data_EEG_STN_Mean-by-Sub_4Perm', med,'.mat']), 'data_eeg', 'data_stn', '-v7.3')

        nSubj = size(data_eeg,1);
        nTime = size(data_eeg,2);
        pvals = nan(1, nTime);

        for t = 1:nTime
            %[~, pvals(t)] = ttest(data_eeg(:,t), data_stn(:,t));
             pvals(t) = signrank(data_on_common(:,t), data_off_common(:,t));
        end
        sig_time = find(pvals < 0.05);
        sig_times = AVGECG.times(sig_time);
        ...
        % --- STN MedOn shaded SEM + line ---
        fill([t, fliplr(t)], ...
            [data_stn_mean(31:end) + data_stn_sem(31:end), ...
            fliplr(data_stn_mean(31:end) - data_stn_sem(31:end))], ...
            [0.2 0.6 1], 'FaceAlpha', 0.3, 'EdgeColor', 'none');  % blue-ish
        h1 = plot(t, data_stn_mean(31:end), 'Color', [0 0.3 0.9], 'LineWidth', 2);

        % --- EEG Med On shaded SEM + line ---
        fill([t, fliplr(t)], ...
            [data_eeg_mean(31:end) + data_eeg_sem(31:end), ...
            fliplr(data_eeg_mean(31:end) - data_eeg_sem(31:end))], ...
            [1 0.4 0.4], 'FaceAlpha', 0.3, 'EdgeColor', 'none');  % red-ish
        h2 = plot(t, data_eeg_mean(31:end), 'Color', [0.9 0.1 0.1], 'LineWidth', 2);

         % --- Variables for significance detection ---
        is_significant = pvals(31:end) < 0.05; % logical vector
        ybounds = [min([data_eeg_mean data_eeg_mean], [], 'all'), ...
            max([data_eeg_mean data_stn_mean], [], 'all')];
        ...
        gr2 = fullfile('E:', 'HeadHeart', '2_results', 'erp', 'clustering', [ 'Hierarchical-Clustering_SIGNIF_Comparison_STN_EEG_HEP_',char(med),'.png']);
        exportgraphics(f2, gr2, "Resolution", 300)

        %
        % t_obs = zeros(1, nTime);
        % for t = 1:nTime
        %     [~, ~, ~, stats] = ttest(data_eeg(:,t), data_stn(:,t));
        %     t_obs(t) = stats.tstat;
        % end
        % 
        % nPerm = 1000;
        % t_perm_max = zeros(1, nPerm);  % store max cluster t-stat per permutation
        % 
        % for p = 1:nPerm
        %     % randomly flip condition per subject
        %     flip = randi([0 1], nSubj, 1)*2 - 1;  % +1 or -1
        %     perm_diff = ((data_eeg - data_stn) .* flip);  % flip signs
        %     t_perm = zeros(1, nTime);
        % 
        %     for t = 1:nTime
        %         [~, ~, ~, stats] = ttest(perm_diff(:,t), zeros(nSubj,1));
        %         t_perm(t) = stats.tstat;
        %     end
        % 
        %     % cluster-based: find contiguous t-values above threshold
        %     thresh = tinv(0.975, nSubj-1);  % two-tailed alpha=0.05
        %     clusters = bwconncomp(abs(t_perm) > thresh); % find contiguous supra-threshold
        %     max_cluster_t = 0;
        %     for c = 1:clusters.NumObjects
        %         max_cluster_t = max(max_cluster_t, sum(t_perm(clusters.PixelIdxList{c})));
        %     end
        %     t_perm_max(p) = max_cluster_t;
        % end
        % 
        % % threshold observed clusters
        % thresh = tinv(0.975, nSubj-1);
        % clusters = bwconncomp(abs(t_obs) > thresh);
        % 
        % sig_idx = false(1, nTime);
        % for c = 1:clusters.NumObjects
        %     cluster_t = sum(t_obs(clusters.PixelIdxList{c}));
        %     % compare to permutation max cluster t
        %     if cluster_t > prctile(t_perm_max, 95)  % cluster p < 0.05
        %         sig_idx(clusters.PixelIdxList{c}) = true;
        %     end
        % end
