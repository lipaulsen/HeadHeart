...
surrogate_rpeaks = zeros(numPerms, length(ibi_series));
for p = 1:numPerms
surrogate_rpeaks(p, :) = ibi_series + (rand(1, length(ibi_series)) - 0.8);
end
...
parfor perm = 1:numPerms
   % % Generate surrogate R-peaks
   % time_shifts = rand(size(ibi_series)) - 0.8;
   % surrogate_rpeaks(perm,:) = ibi_series + time_shifts;
...
function   [EvData] = time_lock_to_surrogate(ChDta, surrogate_rpeaks, SR, tWidth, tOffset, numPerms)
dtTime = 1/SR;
...
function   [surrogate] = surrogate_generation(ibi_series)
time_shifts = rand(size(ibi_series)) - 0.8; % in sec
surrograte = ibi_series + time_shifts; % Temporary variable for each worker
end
