...
FilesDir="C:\UsersData\Lisa\Lucia_StnLFPs_ECG_Lisa\SMR\";
...
FileNames=Lucia_Data_ECG; GrNames="Lucia_Data_ECG"; EvChansTtl = {"EvECGP_Cl"}; ECGChansTtl = {"ECG"}; EvECGChansTtl = {"EvECG"};
...
% IMPORTANT!!!! 
% when you want to calculate ITC then the FIRST
...
%%        
    [fid,WaveChans,EventChans,ChanList]=Smr64_FileOpen(FileNameFull,1);
    if fid <= 0
      stx='Unable to open data file','File Open Error','error';
      disp(stx);
      msgbox(stx);
      return;
    end
%%    
    ...
    ChanListNms=ChanList.ChanNumber;
    ChanListTyp=ChanList.ChanTypeN;
...
%     
    ...
    if stState <= 0; error("Error in Get_ChanNumbersFromTitles"); end
    [ChanNumsEvECG,stState]=Get_ChanNumbersFromTitles(ChanListTts,ChanListNms,EvECGChansTtl);
    if stState <= 0; error("Error in Get_ChanNumbersFromTitles"); end
...
%% Read Event Timings
    EvChanTyp=2;
    [EventTms]=Smr64_ReadEventChan(fid,ChanNumsEv,EvChanTyp);
    nEvs=length(EventTms);
    EvECGChanTyp=2;
    [EvECGTms]=Smr64_ReadEventChan(fid,ChanNumsEvECG,EvECGChanTyp);
%% Read  LFP/EEG Signal
    clear ChanDataWv;
    ich=0;
    for ix=1:nChansWv
...
%% Read in seperate ECG Chan Data 
   ...
   save_path = fullfile('F:\HeadHeart\0_data\itc\evecg' ,strjoin({char(xFileName{1}), '_ECGEV_', char(ECGChansTtl{1}), 'Epoch=-', num2str(tOffset), 'to', num2str(tWidth-tOffset), 's.mat'}, ''));
