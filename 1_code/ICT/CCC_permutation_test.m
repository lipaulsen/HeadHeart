...
parfor perm = 1:numPerms
   ...
   %  surrogate_rpeaks(perm,:) = cumsum(ibi_series) + time_shifts;
    ...
    permCh1 = Ch1EvsFrsTmPha(randperm(nTrials), :, :); % Shuffled data for Channel 1, randperm(nTms)
...
zscores = (FrsTmPsiTrial - diff_sum_perm_mean) ./ diff_sum_perm_std; % this is the imp 
...
histogram(FrsTmPsiTrial);
hold on 
histogram(PermPSICCC);

zscores_thresh = (zscores > 15) | (zscores < -15);

% % Step 2: Compute p-values from z-scores 
% %p_orig = 2 * (1 - normcdf(zscores, 0, 1)); % no abs bc unidirectional 
% %p_orig_cdf = normcdf(zscores, 0, 1); % bc my data was normalized in itself I don't need the two tailed thig
% p_orig = normpdf(zscores, 0, 1);
% 
% % Step 3: Calculate z-scores for permutations
% %p_perm = 2 * (1 - normcdf(zscores_perm, 0, 1)); % no abs bc unidirectional
% p_perm =  normpdf(zscores_perm, 0, 1);
% 
...
% FrsTmPsiTrial_baseline_norm = (FrsTmPsiTrial - baseline_mean); 
% PermPSICCC_baseline_norm = (PermPSICCC - baseline_values_perm); 
...
% [f_original, xi_original] = ksdensity(mean(FrsTmPsiTrial_zscored));
% 
