...
if ismember('Preprocessing ECG', steps)
    ...
    % [B, A] = designNotchPeakIIR(Response ="notch", CenterFrequency= 0.5);
    % ecg_notch_iir = filtfilt(B, A, SmrData.WvDataCropped(16,:));
    ...
    title('Raw data and Notch filter using Fieldtrip');
    xlabel('Time');
    ylabel('Amplitude');
    ...
    plot(SmrData.WvData(16,1:10000), 'k','LineWidth',1)
    hold on
    plot(ecg_data_bandpass_30(1,1:10000),'r','LineWidth',1)
    title('Raw data and bandpass (0.5-30Hz) Filter');
    xlabel('Time');
    ylabel('Amplitude');
    legend('raw trace','bandpass Filter')
    ...
    plot(SmrData.WvData(16,1:10000), 'k','LineWidth',1)
    hold on
    plot(ecg_data_bandpass_40(1,1:10000),'b','LineWidth',1)
    title('Raw data and bandpass (0.5-40Hz) Filter');
    xlabel('Time');
    ylabel('Amplitude');
    legend('raw trace','bandpass Filter')
    ...
    % Apply a 50 Hz notch filter for powerline interference
    d = designfilt('bandstopiir', 'FilterOrder', 2, ...
                   'HalfPowerFrequency1', 49, 'HalfPowerFrequency2', 51, ...
                   'DesignMethod', 'butter', 'SampleRate', sampling_rates.ecg);
    cleaned_ecg = filtfilt(d, cleaned_ecg);
...
if ismember('Preprocessing EEG', steps)
    ...
    % Design Butterworth filters
    lfp_data_bandpass  = ft_preproc_bandpassfilter(SmrData.WvDataCropped(1:15,:), SmrData.SR, [0.1 10],  4, 'but','twopass'); % two pass butterworth filter
