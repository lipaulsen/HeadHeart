...
% Inputs:     Raw EEG + ECG data in .mat files from the raw folder
%
%
% Outputs:    Preprocessed data (EEG, ECG, LFP) in .mat files [SmrData] in preproc folder
%             Plots of preprocessing steps in results preproc folder
...
steps = {'Preprocessing ECG', 'Preprocessing EEG', 'Bipolar Rereferencing'}; %'Formatting', 'Cutting', 'Preprocessing ECG',
...
for sub = 1:numel(subjects) %(medname)
    ...
    fprintf('Processing subject %s number %i of %i, med: %s\n', subject, sub, numel(subjects), medname);
    ...
    % For one Subject the ECG variable was named incorrectly
    if strcmp(subject, 'SG052')
        ecg_idx = find(strcmp(SmrData.WvTits, "ECG2"));
    else
    ...
    if strcmp(subject, 'SG078')
        SmrData.WvTits = cellfun(@char, SmrData.WvTits, 'UniformOutput', false);
    ...
    % Create a clear list with teh EEG and LFP Channels for this subject
    subidx = find(strcmp(subject, {subject_info.ID})); % das ist mein Index
    EEGChans = strsplit(subject_info(subidx).EEG, ', '); % extracting EEG Chans
    LFPChans = strsplit(subject_info(subidx).LFP, ', '); % extracting LFP Chans
    eegidx = find(ismember(SmrData.WvTits, EEGChans));
    lfpidx = find(ismember(SmrData.WvTits,LFPChans));
    ...
    if ismember('Preprocessing ECG', steps)
        ...
        SmrData.WvDataCleaned(end,:) = ecg_data_bandpass; % this is not great yet!!
        SmrData.WvTits{end,1} = 'ECG_filtered';
    ...
    if ismember('Preprocessing EEG', steps)
        ...
        % subtract the activity from the neighboring electrode

        if ismember ('Bipolar Rereferencing', steps)
            LfpElec.SG041  = {'L3', 'R3'};
            LfpElec.SG043  = {'L4', 'R1'};
            LfpElec.SG044  = {'L1', 'R3'};
            LfpElec.SG045  = {'L4', 'R1'};
            ...
            BPRerefLw = zeros(2,size(SmrData.WvDataCleaned,2));
            BPRerefHi = zeros(2,size(SmrData.WvDataCleaned,2));

            for el = 1:2
                currchan = find(strcmp(FltSubsChansRaw{sub},LfpElec.(subject){el}));
                if currchan == 1
                    BipolarReRefLow = 0;
                    BipolarReRefHigh = SmrData.WvDataCleaned(currchan,:) - SmrData.WvDataCleaned(currchan+1,:);
                else
                    BipolarReRefLow = SmrData.WvDataCleaned(currchan,:) - SmrData.WvDataCleaned(currchan-1,:); % Low = the electrode numerically below
                    BipolarReRefHigh = SmrData.WvDataCleaned(currchan,:) - SmrData.WvDataCleaned(currchan+1,:); % High = the electrode numerically above
                end


                BPRerefLw(el,:) = BipolarReRefLow;
                BPRerefHi(el,:) = BipolarReRefHigh;

                if show_plots
                    f6 = figure;
                    set(gcf, 'Position', [1 59 1440 738]);
                    subplot(5,1,1)
                    outlier1 = diff(SmrData.EvData.EvECGP_Cl)> 0.9;
                    outlierdata1 = SmrData.EvData.EvECGP_Cl(outlier1);
                    outlier2 = diff(SmrData.EvData.EvECGP_Cl)> 1.1;
                    outlierdata2 = SmrData.EvData.EvECGP_Cl(outlier2);
                    title('black = 0.9s time difference and red = 1.1 ms time difference')
                    scatter(outlierdata1, 1, 'filled', 'k'), axis('tight')
                    hold on
                    scatter(outlierdata2, 1, 'filled', 'r'), axis('tight')
                    subplot(5,1,2)
                    plot(SmrData.WvDataCleaned(currchan,:), 'k','LineWidth',1)
                    axis('tight');
                    subplot(5,1,3)
                    plot(BipolarReRefLow, 'r','LineWidth',1)
                    axis('tight');
                    subplot(5,1,4)
                    plot(BipolarReRefHigh, 'b','LineWidth',1)
                    axis('tight');
                    subplot(5,1,5)
                    plot(SmrData.WvDataCleaned(currchan,:), 'k','LineWidth',1)
                    hold on
                    ...
                    plot(BipolarReRefHigh, 'b','LineWidth',1)
                    axis('tight');
                    title(['LFP (black) vs. bipolar refrenced one below (red) and one above (blue) sub ' subject, ' Electrode ', LfpElec.(subject){el}]);

                    gr6 = fullfile(results_dir, preprocessed_name, 'bpreref', [subject, '_', medname ,'_bpreref_EEG_raw-vs-below-vs-above_El=', LfpElec.(subject){el},'.png']);
                    exportgraphics(f6, gr6, "Resolution",300)
                end
            end

            % Put the reref things into the smrdata

            if sum(strcmp(subject, {subject_info([subject_info.new]== 1).ID})) == 1 % Check if the curr subject is one of the new ones
                SmrData.WvDataBPRerefHi = vertcat(SmrData.WvDataCleaned(eegidx,:), BPRerefHi, SmrData.WvDataCleaned(end,:));
                SmrData.WvDataBPRerefLow = vertcat(SmrData.WvDataCleaned(eegidx,:), BPRerefLw, SmrData.WvDataCleaned(end,:));
                SmrData.WvTitsBPReref = horzcat(SmrData.WvTitsCleaned(eegidx,:)', LfpElec.(subject), SmrData.WvTitsCleaned(end,:));
            elseif sum(strcmp(subject, {subject_info([subject_info.new]== 1).ID})) == 0
                SmrData.WvDataBPRerefHi = vertcat(SmrData.WvDataCleaned(eegidx,:), BPRerefHi, SmrData.WvDataCleaned(end,:));
                SmrData.WvDataBPRerefLow = vertcat(SmrData.WvDataCleaned(eegidx,:), BPRerefLw, SmrData.WvDataCleaned(end,:));
                SmrData.WvTitsBPReref = horzcat(SmrData.WvTitsCleaned(eegidx,:)', LfpElec.(subject), SmrData.WvTitsCleaned(end,:));
            end
    ...
    if ismember('Bipolar Rereferencing', steps)
        save_path = fullfile(data_dir, preprocessed_name, 'all', [subject,  '_preprocessed_', medname ,'_BPReref_Rest_EEG_HP=', num2str(low_frequency), '-LP=',num2str(high_frequency), '_ECG_HP=', num2str(low_frequency_ecg), '_LP=', num2str(high_frequency_ecg), '_cutoff=', num2str(cut_off_seconds),'s.mat']);
    else
