...
for fn = 2%:2 nur medon gerade
    subfname = subfnames{fn};
    for sub = 1:numel(subjects.new) %(subfname)
        ...
        if ismember('Preprocessing EEG', steps)
            ...
            lfp_data_highpass  = ft_preproc_highpassfilter(SmrData.WvData(1:????,:), SR, low_frequency,  4, 'but','twopass'); % two pass butterworth filter lfp_notch_ft(1:15,:)
            lfp_data_lowpass  = ft_preproc_lowpassfilter(lfp_data_highpass(1:????,:), SR, high_frequency,  4, 'but','twopass'); % two pass butterworth filter
