    ...
    for sub = 1:numel(subjects)
        ...
        for c = 1:numel(channels)
            ...
            if BPRerefHi & onlystn
                save_path = fullfile(data_dir, 'itc', 'ss_chan', [subject, '_', channel, '_ITC_', medname , '_OnlySTN_BP=', BPRerefHiTit, '_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            elseif  BPRerefLw & onlystn
                save_path = fullfile(data_dir, 'itc', 'ss_chan',[subject, '_', channel, '_ITC_', medname , '_OnlySTN_BP=', BPRerefLwTit, '_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            elseif onlyeeg
                save_path = fullfile(data_dir, 'itc', 'ss_chan',[subject, '_', channel, '_ITC_', medname ,'_OnlyEEG_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            elseif allchans & BPRerefHi
                save_path = fullfile(data_dir, 'itc', 'ss_chan',[subject, '_', channel, '_ITC_', medname ,'_BP=', BPRerefHiTit,'_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            elseif allchans & BPRerefLw
                save_path = fullfile(data_dir, 'itc', 'ss_chan',[subject, '_', channel, '_ITC_', medname ,'_BP=', BPRerefLwTit,'_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            elseif allchans
                save_path = fullfile(data_dir, 'itc', 'ss_chan',[subject, '_', channel, '_ITC_', medname ,'_BP=NONE','_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), '_HP=', num2str(freqs(1)) ,'.mat']);
            else
                save_path = fullfile(data_dir, 'itc','ss_chan', [subject,'_', channel, '_ITC_', medname ,'_time=', num2str(times(1)),'-', num2str(times(end)),'_DS=', num2str(SR), 'HPF=', num2str(freqs(1)), '_HP=',  num2str(freqs(1)) ,'.mat']);
            end
            save(save_path, 'ItcDataCh', 'SR', 'times', 'freqs', '-v7.3');
            fprintf('Saved ITC Data for sub %s and channel %s to: %s\n', subject, channel, save_path);
...
if ismember('Group Load TTest by med', steps)
    ...
    pro = {'Load','TTest SC', 'TTest Clus' }; %'Change and Save',  'TTest SC', 'TTest Clus', 'Extract and Plot Peak'
    ...
    if ismember('Change and Save', pro)
        ...
        for c = 1:numel(allChannels)
            ...
            for s = 1:numel(subjects)
                ...
                if startsWith(chan,'L') || startsWith(chan,'R')
                    ...
                    if use_hi_L == 1 || use_hi_R == 1
                        filelist = dir(fullfile(data_dir, 'itc', 'ss_chan', [char(subject), '*', char(chan), '*', medname, '*BPRerefHi', '*.mat']));
                    elseif use_hi_L == 0 || use_hi_R == 0
                        filelist = dir(fullfile(data_dir, 'itc', 'ss_chan', [char(subject), '*', char(chan),  '*',medname, '*BPRerefLow', '*.mat']));
                    end
                else
                    % EEG Files us always BPReref Hi files for ease but BPReref
                    % wasnt done on them
                    filelist = dir(fullfile(data_dir, 'itc', 'ss_chan', [char(subject), '*',  chan,'*', medname, '*BPRerefHi', '*.mat']));
                end
                ...
                % Load ITC
                load(fullfile(filelist(1).folder, filelist(1).name), 'ItcDataCh', 'times', 'freqs', '-mat');

                % Preallocate after first file
                if isempty(allITC)
                    nFreq = size(ItcDataCh,1);
                    nTime = size(ItcDataCh,2);
                    allITC = nan(numel(subjects), nFreq, nTime, numel(allChannels));
                end

                % Store data
                allITC(s,:,:,c) = squeeze(ItcDataCh);
            end
            ...
            if save_dta
                ...
                % create save path
                out_fname = sprintf('%s_AllSubs_ITC_%s_time=-0.3-0.6_DS=300_HP=0.5.mat', char(chan), medname);
                out_path = fullfile(data_dir, 'itc' ,'group_chan', out_fname);

                % Extract per-channel data
                AllSubOneChanITC = squeeze(allITC(:,:,:,c));

                % Save to file (use -v7.3 for large arrays)
                save(out_path, 'AllSubOneChanITC', 'freqs', 'times');
            end
    ...
    if ismember('Load', pro)
        ...
        for c = 1:numel(allChannels)
            ...
            filelist = dir(fullfile(data_dir, 'itc', 'group_chan', [chan, '*.mat']));
            load(fullfile(filelist(1).folder, filelist(1).name), 'AllSubOneChanITC', 'times', 'freqs', '-mat');
            MedOffItcDta(:,:,:,c) = AllSubOneChanITC;
            load(fullfile(filelist(2).folder, filelist(2).name), 'AllSubOneChanITC', 'times', 'freqs', '-mat');
    ...
    if ismember('TTest SC', pro)
        ...
        for c = 1:numel(allChannels)
            ...
            MedOffItcDataSC = squeeze(MedOffItcDta(:,:,:,c));
            MedOnItcDataSC = squeeze(MedOnItcDta(:,:,:,c));
            ...
            MedOnItcDataSC = MedOnItcDataSC(idx,:,:);
            if c <= 10; MedOnItcDataSC(9,:,:) = []; MedOffItcDataSC(9,:,:) = []; end

            % --- Paired t-test at each (freq, time) ---
            for f = 1:numel(freqs)
                for t = 1:numel(times)
                    [~, p, ~, stats] = ttest(squeeze(MedOnItcDataSC(:,f,t)), squeeze(MedOffItcDataSC(:,f,t)));
            ...
            meanDiffMedItc = squeeze(mean(MedOnItcDataSC - MedOffItcDataSC,1));
            ...
            xline(0, "--k", 'LineWidth', 2);
            clim(clims);
            title(sprintf('ITC Difference %s MedOn - MedOff, t(%i)=%.4g, p<%.4g, d=%.4g', chan, stats.df, mean_t, signif_thresh, mean_d))
            xlabel('Time (s)') % Add x-label
            ylabel('Frequencies (Hz)') % Add y-label
            hold off
    ...
    if ismember('TTest Clus', pro)
        ...
        for r = 1:numel(regionNames)
            ...
            xline(0, "--k", 'LineWidth', 2);
            clim(clims);
            title(sprintf('ITC Difference %s MedOn - MedOff, t(%i)=%.4g, p<%.4g, d=%.4g', regName, stats.df,  mean_t, signif_thresh, mean_d))
            xlabel('Time (s)') % Add x-label
            ylabel('Frequencies (Hz)') % Add y-label
            hold off
